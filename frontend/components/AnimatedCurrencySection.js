...
const AnimatedCurrencySection = () => {
  ...
  const loadCurrencies = useCallback(async (isRefresh = false) => {
  ...
  useEffect(() => {
    if (autoRefreshEnabled) {
      ...
      startCurrencyAutoRefresh(handleAutoRefresh, 60); // 60 segundos

      return () => {
        stopCurrencyAutoRefresh();
      };
    }
  }, [autoRefreshEnabled, loadCurrencies, currencies, slideAnim]);
  ...
  // Renderizar card de moeda
  const renderCurrencyCard = ({ item, index }) => (
    <AnimatedCurrencyCard
      currency={item}
      index={index}
      previousCurrency={getPreviousCurrency(item)}
    />
  );

  // Calcular estatísticas do mercado
  const getMarketStats = () => {
    if (currencies.length === 0) return { up: 0, down: 0, total: 0, crypto: 0 };
    
    const up = currencies.filter(c => c.trend === 'up').length;
    const down = currencies.filter(c => c.trend === 'down').length;
    const crypto = currencies.filter(c => ['BTC', 'ETH'].includes(c.code)).length;
    
    return { up, down, total: currencies.length, crypto };
  };
    ...
    <View style={styles.container}>
      ...
      {currencies.length > 0 && (
        <View style={styles.statsContainer}>
          ...
          <View style={[styles.statItem, styles.statHighlight]}>
            <Text style={styles.statNumber}>{stats.crypto}</Text>
      ...
      {loading && currencies.length === 0 ? (
        ...
        <View style={styles.emptyContainer}>
          ...
          <TouchableOpacity onPress={() => loadCurrencies(true)} style={styles.retryButton}>
      ...
      {autoRefreshEnabled && (
        <View style={styles.autoRefreshIndicator}>
          ...
          <Text style={styles.autoRefreshText}>Auto-refresh: 1 min</Text>
        </View>
      )}
    </View>
  );
...
export default AnimatedCurrencySection;
